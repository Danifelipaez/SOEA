...
| HC-C01: Conflicto de grupo | Para cada par de sesiones con el mismo grupo: `timeVar(s₁) ≠ timeVar(s₂)` |
...
| HC-S02: Capacidad | `enrolledStudents(grp(s)) ≤ capacity(r)` — se impone restringiendo el dominio de `spaceVar(s)` a espacios válidos |
...
- Full domain model: sesiones, docentes, grupos, espacios, espacios de tiempo
